...
The database schema implements the data model described in the PRD. Key architectural decisions:
- UUID primary keys for all tables
- Timestamps for all entities (`created_at`, `updated_at`)
- JSON columns for flexible settings
- Array columns for witnesses
...
From [core-concepts.md](../../prd/core-concepts.md), [information-architecture.md](../../prd/information-architecture.md):
- Campaigns, Characters, Scenes, Posts
- Witness-based visibility
- Phase states and pass mechanics
- Compose locks and drafts

---

## Migration File

Create **supabase/migrations/20240101000000_initial.sql**:
...
# Or run via SQL Editor in Dashboard
```
...
1. **Orphaned characters**: `character_assignments` deletion leaves `character_id` intact in posts
2. **Deleted campaigns**: Cascade deletes all related data
3. **User deletion**: Set owner_id to NULL, delete memberships (cascade)
4. **Scene archival**: Only changes `is_archived` flag, data preserved

---

## Next Step

Proceed to [05-ci-cd.md](./05-ci-cd.md) to set up the CI/CD pipeline.
