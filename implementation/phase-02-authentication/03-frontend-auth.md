...
**Goal**: Implement login, registration, and auth callback pages.

---

## Overview

The frontend auth UI includes:
...
export default function Login() {
  ...
  async function handleOAuthLogin(provider: 'google' | 'discord') {
    ...
    try {
      ...
      // OAuth redirects to provider, no need to navigate
    } catch (err) {
    ...
    <div className="flex min-h-screen items-center justify-center px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl">Welcome to Vanguard PBP</CardTitle>
...
- [ ] Registration shows confirmation screen
- [ ] OAuth buttons redirect to provider
- [ ] Auth callback handles success
- [ ] Auth callback shows error gracefully
- [ ] Protected routes redirect to login
- [ ] Protected routes preserve intended destination
...
1. **Email already registered**: Clear error message
2. **OAuth account exists with different provider**: Guidance to user
3. **Verification link expired**: Option to resend
4. **Network failure**: Retry button
5. **Password reset for non-existent email**: Same success message (security)

---

## Phase 2 Complete

With authentication implemented, Phase 2 is complete. You now have:
...
Proceed to [Phase 3: Campaign Core](../phase-03-campaigns/README.md).
